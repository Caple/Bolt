...
import pw.caple.bolt.ApplicationXML.Map.Content;
import pw.caple.bolt.ApplicationXML.Map.Socket;
import pw.caple.bolt.api.Application;

public class ApplicationOnServer {
	...
	private Application appEntry;

	//TODO: make protocol registry work from inside a jar
	//TODO: protocol registry per app
	...
	public ApplicationOnServer(Server server, File root) {
		this.server = server;
		this.root = root;
		name = root.getName();
	}
	...
	private void loadJavaApplication() throws AppLoadException {
		try {
			...
			appEntry = (Application) mainClass.newInstance();
	...
	private void loadIntoWebServer() throws AppLoadException {
		...
		if (xml.map.servlet != null) {
			...
			for (pw.caple.bolt.ApplicationXML.Map.Servlet servletInfo : xml.map.servlet) {
		...
		if (xml.map.socket != null) {
			...
			for (Socket socketInfo : xml.map.socket) {
				...
				sockets.addServlet(Websocket.class, socketInfo.url);
			}
